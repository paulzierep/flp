...
def comp_with_origin(template_smiles, comp_smiles, size, save_path, ori):
    ...
    for bond in new_mol2.GetBonds():
        not_highlight_bond.append(int(bond.GetProp('old_idx'))) #get the bonds not to highlight becouse they are part of the commen structure
    ...
    #when the bond type is changed the bond indices change as well, this is bad, as the bong_highlight pattern in done for the original mol,
...
# t_smiles = 'CC=CC(O)CC(O)CC(=O)C(C)C=C(C)C(=O)CC=O'
# c_smiles = 'CC(CC(C)(O)C(=O)C(O)C(O)C(C(C)O)C(O)(CO)CC(C)C(C)(C)O)C(=O)C(=CO)C(=O)O'
